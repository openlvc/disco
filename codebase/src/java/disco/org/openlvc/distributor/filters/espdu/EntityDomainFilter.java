...
public class EntityDomainFilter extends AbstractEntityStateFilter implements IFilter
{
	//----------------------------------------------------------
	//                    STATIC VARIABLES
	//----------------------------------------------------------
	...
	public EntityDomainFilter( Operator operator, String value )
	{
		...
		this.domain = Domain.valueOf( value );
	}
	...
	public final boolean matches( PDU pdu )
	{
		...
		if( espdu == null )
			...
			return domain.value() == espdu.getEntityType().getDomain();
	}
