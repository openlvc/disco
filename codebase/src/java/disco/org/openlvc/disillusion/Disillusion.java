...
import org.openlvc.disillusion.paths.PathFactory;

public class Disillusion
{
	...
	//                      CONSTRUCTORS
	//----------------------------------------------------------
	protected Disillusion( Configuration configuration )
	{
		this.configuration = configuration;
		this.logger = this.configuration.getDisillusionLogger();
...
//		"[     1]     Detonation -- 0"
	...
	private void populateEntityStore()
	{
		...
		for( int idx = 0; idx < pathDefs.size(); idx++ )
		{
			...
			IPath entityPath = PathFactory.fromJson( JsonUtils.getJSONObject( entry, PATH_DEFINITION, null ), 
			                                         this.locationLookup );			
	...
	private void printWelcome()
	{
		...
		logger.info( "|   \\_ _/ __)" );
		logger.info( "| |) | |\\__ \\ o  ) )     _ o  _   _  " );
		logger.info( "|___/___|___/ ( ( ( (_( (  ( (_) ) ) " );
		logger.info( "                        _)" );
		logger.info( "" );
		logger.info( "Welcome to DISillusion - a tool for creating the illusion of 'real' DIS traffic" );
		logger.info( "" );
