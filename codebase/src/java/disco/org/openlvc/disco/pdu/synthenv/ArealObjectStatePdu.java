/*
 *   Copyright 2025 Open LVC Project.
 *
 *   This file is part of Open LVC Disco.
 *
 *   Licensed under the Apache License, Version 2.0 (the "License");
 *   you may not use this file except in compliance with the License.
 *   You may obtain a copy of the License at
 *
 *       http://www.apache.org/licenses/LICENSE-2.0
 *
 *   Unless required by applicable law or agreed to in writing, software
 *   distributed under the License is distributed on an "AS IS" BASIS,
 *   WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *   See the License for the specific language governing permissions and
 *   limitations under the License.
 */
package org.openlvc.disco.pdu.synthenv;

import java.io.IOException;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;
import java.util.Objects;

import org.openlvc.disco.pdu.DisInputStream;
import org.openlvc.disco.pdu.DisOutputStream;
import org.openlvc.disco.pdu.PDU;
import org.openlvc.disco.pdu.field.ForceId;
import org.openlvc.disco.pdu.field.PduType;
import org.openlvc.disco.pdu.field.appearance.GeneralObjectApperance;
import org.openlvc.disco.pdu.record.ObjectId;
import org.openlvc.disco.pdu.record.ObjectType;
import org.openlvc.disco.pdu.record.SimulationAddress;
import org.openlvc.disco.pdu.record.WorldCoordinate;

public class ArealObjectStatePdu extends PDU
{
	//----------------------------------------------------------
	//                    STATIC VARIABLES
	//----------------------------------------------------------

	//----------------------------------------------------------
	//                   INSTANCE VARIABLES
	//----------------------------------------------------------
	private ObjectId objectId;
	private ObjectId referencedObjectId;
	private int updateNumber;
	private ForceId forceId;
	private byte modifications;
	private ObjectType objectType;
	private byte[] specificAppearance; // Spec just says 32-bit record, no impl detail given
	private GeneralObjectApperance generalApperance;
	private SimulationAddress requestorId;
	private SimulationAddress receivingId;
	private List<WorldCoordinate> points;

	//----------------------------------------------------------
	//                      CONSTRUCTORS
	//----------------------------------------------------------
	public ArealObjectStatePdu()
	{
		super( PduType.ArealObjectState );
		this.objectId = new ObjectId();
		this.referencedObjectId = new ObjectId();
		this.updateNumber = -1;
		this.forceId = ForceId.Other;
		this.modifications = 0;
		this.objectType = new ObjectType();
		this.specificAppearance = new byte[4];
		this.generalApperance = new GeneralObjectApperance();
		this.requestorId = new SimulationAddress();
		this.receivingId = new SimulationAddress();
		this.points = new ArrayList<>();
	}

	//----------------------------------------------------------
	//                    INSTANCE METHODS
	//----------------------------------------------------------
	@Override
	public boolean equals( Object other )
	{
		if( other == null )
			return false;
		
		if( other instanceof ArealObjectStatePdu )
		{
			ArealObjectStatePdu otherAos = (ArealObjectStatePdu)other;
			return Objects.equals( this.objectId, otherAos.objectId ) &&
			       Objects.equals( this.referencedObjectId, otherAos.referencedObjectId ) &&
			       this.updateNumber == otherAos.updateNumber &&
			       this.forceId == otherAos.forceId &&
			       this.modifications == otherAos.modifications &&
			       Objects.equals( this.objectType, otherAos.objectType ) &&
			       Arrays.equals( this.specificAppearance, otherAos.specificAppearance ) &&
			       Objects.equals( this.generalApperance, otherAos.generalApperance ) &&
			       Objects.equals( this.requestorId, otherAos.requestorId ) &&
			       Objects.equals( this.receivingId, otherAos.receivingId ) &&
			       Objects.equals( this.points, otherAos.points );
		}
		else
		{
			return false;
		}
	}
	
	@Override
	public int hashCode()
	{
		return Objects.hash( this.objectId, 
		                     this.referencedObjectId, 
		                     this.updateNumber,
		                     this.forceId,
		                     this.modifications,
		                     this.objectType,
		                     this.specificAppearance,
		                     this.generalApperance,
		                     this.requestorId,
		                     this.receivingId,
		                     this.points );
	}
	
	@Override
	public void from( DisInputStream dis ) throws IOException
	{
		this.objectId.from( dis );
		this.referencedObjectId.from( dis );
		this.updateNumber = dis.readUI16();
		this.forceId = ForceId.fromValue( dis.readUI8() );
		this.modifications = dis.readByte();
		this.objectType.from( dis );
		dis.readFully( this.specificAppearance );
		this.generalApperance.setBits( dis.readUI16() );
		int numberOfPoints = dis.readUI16();
		this.requestorId.from( dis );
		this.receivingId.from( dis );
		
		this.points.clear();
		for( int i = 0 ; i < numberOfPoints ; ++i )
		{
			WorldCoordinate pointI = new WorldCoordinate();
			pointI.from( dis );
			this.points.add( pointI );
		}
	}
	
	@Override
	public void to( DisOutputStream dos ) throws IOException
	{
		this.objectId.to( dos );
		this.referencedObjectId.to( dos );
		dos.writeUI16( this.updateNumber );
		dos.writeUI8( this.forceId.value() );
		dos.writeByte( this.modifications );
		this.objectType.to( dos );
		dos.write( this.specificAppearance );
		dos.writeUI16( this.generalApperance.getBits() );
		dos.writeUI16( this.points.size() );
		this.requestorId.to( dos );
		this.receivingId.to( dos );
		for( WorldCoordinate point : this.points )
			point.to( dos );
	}
	
	@Override
	public final int getContentLength()
	{
		return 48 + (this.points.size() * 24);
	}

	////////////////////////////////////////////////////////////////////////////////////////////
	/// Convenience Methods   //////////////////////////////////////////////////////////////////
	////////////////////////////////////////////////////////////////////////////////////////////
	@Override
	public int getSiteId()
	{
		return objectId.getSiteId();
	}

	@Override
	public int getAppId()
	{
		return objectId.getAppId();
	}

	public ObjectId getObjectId()
	{
		return objectId;
	}

	public void setObjectId( ObjectId objectId )
	{
		this.objectId = objectId;
	}

	public ObjectId getReferencedObjectId()
	{
		return referencedObjectId;
	}

	public void setReferencedObjectId( ObjectId referencedObjectId )
	{
		this.referencedObjectId = referencedObjectId;
	}

	public int getUpdateNumber()
	{
		return updateNumber;
	}

	public void setUpdateNumber( int updateNumber )
	{
		this.updateNumber = updateNumber;
	}

	public ForceId getForceId()
	{
		return forceId;
	}

	public void setForceId( ForceId forceId )
	{
		this.forceId = forceId;
	}

	public byte getModifications()
	{
		return modifications;
	}

	public void setModifications( byte modifications )
	{
		this.modifications = modifications;
	}

	public ObjectType getObjectType()
	{
		return objectType;
	}

	public void setObjectType( ObjectType objectType )
	{
		this.objectType = objectType;
	}

	public byte[] getSpecificAppearance()
	{
		return specificAppearance;
	}

	public void setSpecificAppearance( byte[] specificObjectAppearance )
	{
		if( specificObjectAppearance.length != 4 )
			throw new IllegalArgumentException( "expected 32bit value for specific appearance" );
		this.specificAppearance = specificObjectAppearance;
	}

	public GeneralObjectApperance getGeneralApperance()
	{
		return generalApperance;
	}

	public void setGeneralApperance( GeneralObjectApperance generalObjectApperance )
	{
		this.generalApperance = generalObjectApperance;
	}
	
	public SimulationAddress getRequestorId()
	{
		return requestorId;
	}

	public void setRequestorId( SimulationAddress requestorId )
	{
		this.requestorId = requestorId;
	}

	public SimulationAddress getReceivingId()
	{
		return receivingId;
	}

	public void setReceivingId( SimulationAddress receivingId )
	{
		this.receivingId = receivingId;
	}

	public List<WorldCoordinate> getPoints()
	{
		return new ArrayList<>( this.points );
	}

	public void setPoints( List<WorldCoordinate> points )
	{
		this.points = new ArrayList<>( points );
	}
	
	////////////////////////////////////////////////////////////////////////////////////////////
	/// Accessor and Mutator Methods   /////////////////////////////////////////////////////////
	////////////////////////////////////////////////////////////////////////////////////////////
	
	//----------------------------------------------------------
	//                     STATIC METHODS
	//----------------------------------------------------------
}
